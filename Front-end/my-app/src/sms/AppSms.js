...
export default class AppSms extends React.Component {
    constructor(props){
        super(props);
        this.state = {
            code: '',
    ...
    _getCode = async() => {
        ...
        await axios.get("http://localhost:4500/getcode", {
            params: {
                phonenumber: e,
                channel: 'sms'
            }
    ...
    _verifyCode = async () => {
        const e = this.state.code+this.state.pno;
       const resp= await axios.get("http://localhost:4500/verifycode", {
            params: {
                phonenumber: e,
                code: this.state.otp,
            }
    ...
    render(){
        return(
            <div style={{
                ...
                <Paper elevation={4} style={{ padding: 20, width: 400, marginBottom: 60}}>
                    {!this.state.otpShow ? <h3 style={{marginLeft: 2, color: '#9f9f9f'}}>Housing Society</h3> : <IconButton onClick={() => {
                    ...
                    <div>
                        ...
                        {!this.state.otpShow ? <p>By tapping Verify an SMS may be sent.</p> : null}
